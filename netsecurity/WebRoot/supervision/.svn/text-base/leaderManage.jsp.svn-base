...
$(function(){
    ...
    $grid.datagrid({
	   ...
	   columns: [[
                /*{checkbox: true},*/
                {field: 'userid', title: '学工号',width:wid * 0.2,formatter:function(value,row,index){
					return '<a href="showComputerOnLinePage.action?userId='+row.userid+'">'+value+'</a>'+'<a href="#" onclick="openLineChart1(\'dlg\', \''+row.userid+'实验室行为统计\', \'dlg-toolbar\', \'#00C000\', \'canvas\','+row.labId+');"><img title="电脑状态线图" width="16" height="16" style="margin-left:5px;" border="0" src="enterUploadPage_files/line.png"></a>';}, sortable: true},
				{field: 'username', title: '姓名',formatter:function(value){
					return '<span class="blue">'+value+'</span>';},width:wid * 0.2},
				{field: 'coperation', title: '操作',formatter:function(value,row,index){
					return '<span title="删除" class="img-btn icon-remove" onClick="del(\''+row.userid +'\')"></span>'
                        		+'<a style="text-decoration:none" href="javascript:void(0)" onClick="del(\''+ row.userid +'\')">删除</a>&nbsp;&nbsp;';
...
function opendlg(title, href, form_id)
{
	$("#dlg").dialog({
		...
		buttons: [{
					...
					iconCls:'icon-ok',
					handler:function(){
					var url="addLeader.action";
					var params={'searchStr':$('#userid').attr('value')};
...
//删除组长
function del(id){
	var url='delLeader.action';

	var parameters={
		'userId':id
		}
	$.post(url,parameters,callDelBack,'html');
}
function callDelBack(value){
	result=eval(value);
	if(result==true)
	 {
	 	alert('删除成功');
...
//查询 组长 
	function submitSearchInfo() {
		//alert("submitSearchInfo!");
		var params = $('#operation_log_grid').datagrid('options').queryParams;  
        params.page = 1;  
        params.rows = 10;      
        params.searchStr=$('#searchStr').attr('value');
        //alert("params: " +  params);
        $('#operation_log_grid').datagrid('options').queryParams = params;   
        //load	加载并显示第一页的行，如果指定 param 参数，它将替换 queryParams 特性。
   		$('#operation_log_grid').datagrid('load'); 
   		//reload 重新加载行，就像 load 方法一样，但是保持在当前页。
   		//$("#operation_ServerSecurity_grid").datagrid('reload'); 
	}
    ...
    <form id="bt_user_search_from" style="height:50px; overflow:hidden; margin:0;">
      <table style="height:50px;margin-left:22px;" >
        <tr>
          ...
          <td><input name="searchStr" id="searchStr" type="text"></td>
...
<div id="dlg3" style="padding:5px;"> </div>
</html>
