...
$(function(){
    ...
    $grid.datagrid({
		...
		fitColumns: true,
        idField: 'tools',
        url: 'showGroupUsersOnLineData.action?groupId=${session.groupId}',
        pagination: true,
		pageSize: 10,
		rownumbers:true,
		singleSelect:true,
		...
		sortOrder:"desc",
			...
			disabled:false,
			handler:function(){
				window.location = "groupManage.jsp";
			}
		},{
			text:'首页',
	   ...
	   columns: [[
				...
				{field: 'username',title:'姓名',formatter:function(value,row,index){
					return '<a href="showComputerOnLinePage.action?userId='+row.userid+'">'+value+'</a>'+'<a href="#" onclick="openLineChart1(\'dlg\', \''+value+'电脑行为统计\', \'dlg-toolbar\', \'#00C000\', \'canvas\',\''+row.userid+'\');"><img title="电脑状态线图" width="16" height="16" style="margin-left:5px;" border="0" src="enterUploadPage_files/line.png"></a>';
				},width:wid*0.2},
				{field: 'totalNum', title: '总电脑',formatter:function(value){
					return '<span class="blue">'+value+'</span>';},width:wid * 0.2},
				{field: 'onlineNum', title: '实时在线电脑',formatter:function(value){
					return '<span class="green2">'+value+'</span>';},width:wid * 0.2},
				{field: 'offlineNum', title: '离线电脑',formatter:function(value){
...
function trafficChange1(){
		...
		var url='getUserStatusTrendDataByDay.action';
		var userid=$('#userid').attr('value');
...
<!--  	 <span class="welcomeAdm">管理员：<b>${session.user.username }</b>您好，欢迎登陆使用！<a href="logout.action" class="exitLo">退出登陆</a></span>-->
     <span class="tableName"><strong>用户信息统计</strong></span>
  </div>
